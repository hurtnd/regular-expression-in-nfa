...
def infix_to_postfix(infix):
    ...
    for char in infix:
        ...
        # Если символ - открывающая скобка, он добавляется в стек
            ...
            stack.append(char)
        # Если символ - закрывающая скобка, извлекаются все операторы из стека и
        # добавляются в выходную строку до тех пор, пока не встретится открывающая скобка
        elif char == ')':
...
def match(infix, word):
    ...
    """
    # Преобразуем регулярное выражение из инфиксной нотации в постфиксную нотацию
...
def is_correct_regex(regex_str):
    ...
    """
    if len(regex_str) > 1:
        # Проверяем, что регулярное выражение начинается со скобки и заканчивается скобкой
        if regex_str[0] == '(' and regex_str[-1] == ')':
            # Создаем новую строку, которая содержит среднюю часть регулярного выражения, 
            ...
            for i in range(0, len(middle)):
                if middle[i] == CONCATENATION or middle[i] == UNION:
                    left = middle[0:i]  # Строка, которая содержит левую часть текущего подвыражения
                    right = middle[i + 1:]  # Строка, которая содержит правую часть текущего подвыражения
                    if is_correct_regex(left) and is_correct_regex(right):
                        return True
            return False
        # Проверяем, что регулярное выражение заканчивается символом звездочки Клини
